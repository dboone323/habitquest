...
LOG_FILE="$(dirname "$0")/agent_debug.log"
...
function update_status() {
  ...
  while [[ ${attempt} -lt ${max_attempts} ]]; do
    ...
    if command -v jq &>/dev/null &&
      ...
      current_time=$(date)
      echo "[${current_time}] ${AGENT_NAME}: Failed to update agent_status.json (jq or mv error)" >>"${LOG_FILE}"
...
while true; do
  ...
  update_status idle
  current_time=$(date)
  echo "[${current_time}] ${AGENT_NAME}: Cycle completed. Sleeping for ${SLEEP_INTERVAL} seconds." >>"${LOG_FILE}"
