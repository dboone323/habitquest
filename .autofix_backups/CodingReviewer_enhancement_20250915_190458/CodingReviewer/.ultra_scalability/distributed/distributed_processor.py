...
class LoadBalancer:
    ...
    def get_optimal_node(self) -> ProcessingNode:
        ...
        available_nodes = [
            ...
            if node.status == "idle" or node.current_load < 0.8
        ]
...
class DistributedProcessor:
    ...
    async def process_task_on_node(self, node: ProcessingNode, task: ProcessingTask):
        ...
        try:
            ...
            processing_time = task.estimated_duration * complexity_multiplier.get(
                task.complexity, 1.0
            )
        ...
        self.metrics.update(
            {
                ...
                "success_rate": (
                    len(successful_tasks) / len(results) * 100 if results else 0
                ),
                ...
                "node_utilization": {
                    node.node_id: {
                        ...
                        "utilization": node.processed_tasks
                        / (len(results) / len(self.nodes))
                        * 100,
                    }
                    for node in self.nodes
                },
...
async def main():
    ...
    for node_id, utilization in results["node_utilization"].items():
        print(
            f"  {node_id}: {utilization['processed_tasks']} tasks ({utilization['utilization']:.1f}%)"
        )
    ...
    if results["success_rate"] >= 95 and results["throughput"] >= 100:
        ...
        print(
            f"🎉 Successfully processed {results['total_tasks']} tasks across {len(processor.nodes)} nodes"
        )
        return True
    else:
