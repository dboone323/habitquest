...
ultra_detect_secrets() {
  ...
  # Patterns for various secret types (focus on paid APIs, allow free AI)
  local patterns=(
    "sk-[a-zA-Z0-9]{32,}"                      # OpenAI API keys (PAID)
    'AIza[0-9A-Za-z\-_]{35}'                   # Google API keys (PAID)
    "AKIA[0-9A-Z]{16}"                         # AWS Access Key (PAID)
    "xoxb-[0-9]{11}-[0-9]{11}-[a-zA-Z0-9]{24}" # Slack Bot tokens (PAID)
    "sk-ant-[a-zA-Z0-9]{95}"                   # Anthropic/Claude API keys (PAID)
  )

  # Free AI patterns to exclude from warnings
  local free_ai_patterns=(
    "hf_[a-zA-Z0-9]{34,}" # Hugging Face tokens (FREE)
    "ollama"              # Ollama configurations (FREE)
    "localhost:11434"     # Ollama local server (FREE)
  )
    ...
    while IFS= read -r line; do
      ...
      for pattern in "${patterns[@]}"; do
        if echo "$line" | grep -qE "$pattern"; then
          # Check if this is a free AI configuration (exclude from warnings)
          local is_free_ai=false
          for free_pattern in "${free_ai_patterns[@]}"; do
            if echo "$line" | grep -qE "$free_pattern"; then
              is_free_ai=true
              break
            fi
          done

          if [[ $is_free_ai == "true" ]]; then
            continue # Skip free AI configurations
          fi

          # Intelligent context analysis to reduce false positives
          if ! echo "$line" | grep -qE "(example|sample|placeholder|TODO|FIXME|test|demo)"; then
            ((secrets_found++))

            local severity="CRITICAL"
            local description="Paid API key detected - consider using free alternatives like Ollama"

            echo "$(date '+%Y-%m-%d %H:%M:%S') | $severity | paid_api_usage | $file | $line_number | $description | DETECTED" >>"$VULNERABILITIES_LOG"

            echo -e "   ${RED}🚨 CRITICAL: Paid API key in $filename:$line_number${NC}"
            echo -e "      💡 Recommendation: Replace with free Ollama or Hugging Face${NC}"
          fi
        fi
      done
    done <"$file"
