...
final class HabitViewModelTests: XCTestCase {
    ...
    func testCompleteHabit() {
        // Create habit
        viewModel.handle(.createHabit(name: "H1", description: "D1", frequency: .daily, category: .mindfulness, difficulty: .easy))
        guard let habit = viewModel.state.habits.first else { XCTFail("Habit not created"); return }
    ...
    func testDeleteHabit() {
        viewModel.handle(.createHabit(name: "Delete Me", description: "...", frequency: .daily, category: .health, difficulty: .easy))
        guard let habit = viewModel.state.habits.first else { return }
    ...
    func testFilterSearch() {
        viewModel.handle(.createHabit(name: "Read Book", description: "Read", frequency: .daily, category: .mindfulness, difficulty: .easy))
        viewModel.handle(.createHabit(name: "Exercise", description: "Run", frequency: .daily, category: .health, difficulty: .hard))
    ...
    func testFilterCategory() {
        viewModel.handle(.createHabit(name: "H1", description: "D1", frequency: .daily, category: .mindfulness, difficulty: .easy)) // Mindfulness
        viewModel.handle(.createHabit(name: "H2", description: "D2", frequency: .daily, category: .health, difficulty: .easy)) // Health
    ...
    func testTodaysHabits() {
        viewModel.handle(.createHabit(name: "Daily", description: "D", frequency: .daily, category: .health, difficulty: .easy))
