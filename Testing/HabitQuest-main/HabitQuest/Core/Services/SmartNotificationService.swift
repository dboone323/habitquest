...
final class SmartNotificationService {
    ...
    private func generatePersonalizedTitle(for habit: Habit, prediction: StreakPrediction) -> String {
        ...
        case (let s, let p) where s >= 21 && p > 80:
            return "🔥 Keep the \(s)-day streak alive!"
        case (let s, let p) where s >= 7 && p > 70:
            return "💪 \(s) days strong - don't break it now!"
        case (let s, _) where s >= 3:
            return "⭐ \(s)-day streak in progress"
        case (_, let p) where p < 40:
