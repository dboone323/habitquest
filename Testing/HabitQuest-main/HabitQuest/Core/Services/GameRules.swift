...
struct GameRules {
    ...
    private static func updateHabitStreak(habit: Habit) {
        ...
        for i in 1..<sortedLogs.count {
            let currentLog = sortedLogs[i-1]
            let previousLog = sortedLogs[i]

            let isConsecutive: Bool

            switch habit.frequency {
            case .daily:
                let daysBetween = calendar.dateComponents([.day], from: previousLog.completionDate, to: currentLog.completionDate).day ?? 0
                isConsecutive = daysBetween == 1
            case .weekly:
                let weeksBetween = calendar.dateComponents([.weekOfYear], from: previousLog.completionDate, to: currentLog.completionDate).weekOfYear ?? 0
    ...
    /// - Returns: Tuple indicating if level up occurred and new level
    @discardableResult
    static func processXPGain(xp: Int, for profile: PlayerProfile) -> (leveledUp: Bool, newLevel: Int) {
