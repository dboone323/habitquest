...
class ThemeManager: ObservableObject {
    ...
    // Finds the Theme struct corresponding to the name stored in `selectedThemeName`
    // and updates the `currentTheme` published property if it has changed.
    private func updateCurrentTheme() {
        // Find the theme matching the name stored in `selectedThemeName`.
        let newTheme = Theme.availableThemes.first { $0.name == selectedThemeName } ?? Theme.defaultTheme
