...
build_ios() {
    ...
    for strategy in "${strategies[@]}"; do
        ...
        # Try with project file first, then without
        if ls *.xcodeproj 1> /dev/null 2>&1; then
                if CODE_SIGNING_ALLOWED=${CODE_SIGNING_ALLOWED:-NO} xcodebuild -scheme "$scheme" -destination "$strategy" build 2>/dev/null || \
                    CODE_SIGNING_ALLOWED=${CODE_SIGNING_ALLOWED:-NO} xcodebuild -project *.xcodeproj -scheme "$scheme" -destination "$strategy" build; then
...
test_ios() {
    ...
    if CODE_SIGNING_ALLOWED=${CODE_SIGNING_ALLOWED:-NO} xcodebuild test -scheme "$scheme" -destination "platform=iOS Simulator,name=iPhone 16"; then
